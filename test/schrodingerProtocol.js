...
contract("SchrodingerProtocol", function(accounts) {
    const healthData = {
        patient_id_0: {
            genetic_data: {
                rs10757274: "AA",
    ...
    // contracts
    let schrodingerProtocolInstance;
    ...
    beforeEach(async() => {
        ...
        schrodingerProtocolInstance = await SchrodingerProtocol.new(
    ...
    describe("Public keys management with NFTs", async() => {
        it("should allow an authorized data provider with a valid token to add their public key", async() => {
            ...
            await schrodingerProtocolInstance.setPublicKey(name, publicKey, {
                from: providerA,
            });

            // Verify that providerA's public key was set correctly
            const retrievedPublicKey = await schrodingerProtocolInstance.getPublicKey(name, 0);
            assert.equal(
                retrievedPublicKey,
                publicKey,
        ...
        it("should allow an authorized data provider with a valid token to add new public keys", async() => {
            ...
            await schrodingerProtocolInstance.setPublicKey(name, publicKey1, {
                from: providerA,
            });

            // Set providerA's second public key
            await schrodingerProtocolInstance.setPublicKey(name, publicKey2, {
                from: providerA,
            });

            // Set providerA's third public key
            await schrodingerProtocolInstance.setPublicKey(name, publicKey3, {
                from: providerA,
            });

            // Set providerA's fourth public key
            await schrodingerProtocolInstance.setPublicKey(name, publicKey4, {
                from: providerA,
            });

            // Verify that providerA's first public keys were set correctly
            const retrievedPublicKey1 = await schrodingerProtocolInstance.getPublicKey(name, 0);
            assert.equal(
                retrievedPublicKey1,
                publicKey1,
            ...
            const retrievedPublicKey2 = await schrodingerProtocolInstance.getPublicKey(name, 1);
            assert.equal(
                retrievedPublicKey2,
                publicKey2,
            ...
            const retrievedPublicKey3 = await schrodingerProtocolInstance.getPublicKey(name, 2);
            assert.equal(
                retrievedPublicKey3,
                publicKey3,
            ...
            const retrievedPublicKey4 = await schrodingerProtocolInstance.getPublicKey(name, 3);
            assert.equal(
                retrievedPublicKey4,
                publicKey4,
            ...
            const retrievedLatestPublicKey =
                await schrodingerProtocolInstance.getLatestPublicKey(name);
            assert.equal(
        ...
        it("should allow a data provider to update their own public key", async() => {
            ...
            await schrodingerProtocolInstance.setPublicKey(initialName, initialPublicKey, {
                from: providerA,
            });
            ...
            await schrodingerProtocolInstance.setPublicKey(updatedName, updatedPublicKey, {
                from: providerA,
            });

            // Verify that providerA's public key was updated correctly
            const retrievedPublicKey = await schrodingerProtocolInstance.getPublicKey(
                updatedName,
                1
            );
            assert.equal(
        ...
        it("should not allow an unauthorized data provider/user to set a public key", async() => {
            ...
            await expect(
                schrodingerProtocolInstance.setPublicKey(
                    name,
            ...
            await expect(
                schrodingerProtocolInstance.getLatestPublicKey(name)
            ).to.be.rejectedWith(
        ...
        it("should not allow setting a public key with an empty string", async() => {
            ...
            const name = "Provider A";
            await expect(
                schrodingerProtocolInstance.setPublicKey(name, "", { from: providerA })
            ).to.be.rejectedWith(
        ...
        it("should not allow an authorized data provider to modify another authorized data provider's public key with the same name", async() => {
            ...
            await schrodingerProtocolInstance.setPublicKey(
                name,
        ...
        it("should allow an authorized data provider to transfer their token and the new address to update their public key", async() => {
            ...
            await schrodingerProtocolInstance.setPublicKey(name, newPublicKey, {
                from: providerANewAddress,
            });

            // Verify that providerA's new public key was set correctly
            const retrievedNewPublicKey = await schrodingerProtocolInstance.getLatestPublicKey(
                name
            );
            assert.equal(
    ...
    describe("Data access management", async() => {
        it("should allow an authorized provider to set a data source", async() => {
            ...
            await schrodingerProtocolInstance.setDataSource(
                dataSourceId,
                dataSourceUrl,
            ...
            const dataSource = await schrodingerProtocolInstance.getDataSource(dataSourceId, { from: analyst });
        ...
        it("should not allow a data analyst with mismatching data access policy to access the data", async() => {
            ...
            await expect(
                schrodingerProtocolInstance.getDataSource(dataSourceId, { from: analyst })
            ).to.be.rejectedWith(
    ...
    describe("Verification of proofs", async() => {
        ...
        describe("Simplified flow", async() => {
            it("signature stored", async() => {
                ...
                await schrodingerProtocolInstance.storeSignature(publicKey, signature, {
                    from: providerA,
                });

                const signatureStored = await schrodingerProtocolInstance.checkSignature(
                    publicKey,
                    signature
                );
            ...
            it("valid proof of provenance", async() => {
                ...
                const publicKeyResult = await schrodingerProtocolInstance.verifyPublicInputsPoP(
                    publicKey,
                    proof
                );
                expect(publicKeyResult).eq(true);

                const smartContractResult = await schrodingerProtocolInstance.verifyProof(
                    "proof_of_provenance",
                    proof
                );
            ...
            it("invalid proof of provenance", async() => {
                ...
                await expect(
                    schrodingerProtocolInstance.verifyProof("proof_of_provenance", proof)
                ).to.be.rejectedWith(
        ...
        describe("Complete flow", async() => {
            it("valid proof", async() => {
                ...
                const providerPublicKeyP = await schrodingerProtocolInstance.getPublicKey(
                    providerNameP,
                    publicKeyVersionP
                );
                ...
                const providerPublicKeyV = await schrodingerProtocolInstance.getPublicKey(
                    providerNameV,
                    publicKeyVersionV
                );
                ...
                const publicKeyMatch = await schrodingerProtocolInstance.verifyPublicInputsPoP(
                    providerPublicKeyV,
                    proofV
                );
                ...
                const proofVerified = await schrodingerProtocolInstance.verifyProof(
                    "proof_of_provenance",
                    proofV
                );
