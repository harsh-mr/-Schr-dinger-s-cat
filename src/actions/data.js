...
async function getData(from, dataId) {
  const sc = contractsHelper.getContractByName("SchrodingerProtocol");

  try {
    const dataSource = await sc.methods.getDataSource(dataId).call({ from });
...
async function setData(from, dataId, url, credentials, accessPolicies) {
  const sc = contractsHelper.getContractByName("");

  try {
    await sc.methods
      .setDataSource(dataId, url, credentials, accessPolicies)
