...
class ContractsHelper {
    ...
    async addDeployed(contract) {
        const compiledContract = compile(contract.filename, contract.name);
        
        console.log(`Loading ${contract.name} from address ${contract.address}`);

        this.contracts[contract.name] = contract;
        this.contracts[contract.name].address = contract.address;
        this.contracts[contract.name].abi = compiledContract.abi;
        this.contracts[contract.name].contract = new web3.eth.Contract(
