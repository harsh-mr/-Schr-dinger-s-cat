...
async function generateVerifiers(verifierCircuits) {
  for (const circuit of verifierCircuits) {
    ...
    const circuitPath = resolve(
      __dirname,
      `../circuits/${verifierCircuitName}/src/main.nr`
    );
    ...
    if (!fs.existsSync(circuitPath)) {
      console.log(
        `Error: circuit ${verifierCircuitName} (${circuitPath}) does not exist`
      );
      return;
    }
    ...
    try {
      console.log(`Attempting to compile: ${circuitPath}`);
      compiledProgram = compile(circuitPath);
    } catch (e) {
      console.log(`Compilation error details:`);
      console.log(`Circuit: ${verifierCircuitName}`);
    ...
    try {
      ...
      console.log(`Verifier generation error:`, e);
      return;
    }
    ...
    console.log(
      `► ${verifierCircuitName} circuit → ${verifierContractName} smart contract (${verifierPath}) ✓`
    );
  }
}
...
const verifierCircuits = [
  ...
  {
    circuit_name: "average_dot_products", // name of the directory that contains the circuit
    contract_name: "VerifierAverageDotProducts", // name of the smart contract to generated
  },
];
